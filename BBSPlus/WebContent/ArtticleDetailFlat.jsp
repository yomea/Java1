...
<%	
	...
	ResultSet rs = null;
	Article a = null;
	String strId = request.getParameter("rootId");
	int rootId = 0; 	
	if(strId == null) {
		out.println("请通过正确的方式登陆页面！");
		return;
	} else {
		try {
			rootId = Integer.parseInt(strId.trim());
		} catch(NumberFormatException e) {
			out.println("请通过正常的方式登陆页面！");
			return;
		}
		Connection conn = DB.getConn();
		String sql = "Select * from article where rootid=" + rootId +" order by id";
 ...
 %>
  <div class="jive-buttons">
    <table summary="Buttons" border="0" cellpadding="0" cellspacing="0">
      <tbody>
        <tr>
          <td class="jive-icon"><a href="http://bbs.chinajavaworld.com/post%21reply.jspa?threadID=744236"><%=article.getId() == 1 ? "<img src='images/reply-16x16.gif' alt='回复本主题' border='0' height='16' width='16'>" : "" %></a></td>
          <td class="jive-icon-label"><a id="jive-reply-thread" href="reply.jsp?id=<%=article.getId() %>&rootId=<%=article.getRootId()%>"><%=article.getId() == 1 ? "回复本主题" : "" %></a> </td>
        </tr>
      </tbody>
    </table>
  </div>
                      ...
                      <tr id="jive-message-780144" class="jive-odd" valign="top">
                        ...
                        <td class="jive-last" width="99%"><table border="0" cellpadding="0" cellspacing="0" width="100%">
                            <tbody>
                              <tr valign="top">
                                <td width="1%"></td>
                                <td width="97%"><span class="jive-subject"> <%=article.getId() == 1 ? "楼主" : article.getId() + "楼" + " 回复 " + (article.getPid() == 1 ? "楼主" : article.getPid() + "楼") %></span> </td>
                              ...
                              <tr>
                                <td colspan="4" style="border-top: 1px solid rgb(204, 204, 204);"><br><%=article.getCont() %>
                                  <br>
                                  <br>
                                </td>
                              </tr>
                              ...
                              <tr>
                                <td colspan="4" style="border-top: 1px solid rgb(204, 204, 204); font-size: 9pt; table-layout: fixed;"> ·<a href="http://www.bjsxt.com"><font color="#666666">拒绝广告</font></a> </td>
                              </tr>
                            </tbody>
                          </table></td>
                      </tr>
